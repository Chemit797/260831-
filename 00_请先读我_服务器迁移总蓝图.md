...
> 状态：紧急迁移范围已完成：14 个精选研究包与 3 个去凭据 Codex 预快照均已上传到 Drive 并逐包校验。Drive 容量充足后，约 307.5 GiB 的个人研究全量第二波正在上传；明确不包含他人的 mousebrain 项目或可重新下载的 Biohub 比赛数据。没有删除服务器文件。
>
> 更新时间：2026-08-31
...
| 二进制制品层 | Google Drive 指定文件夹 | V7 数据、正式模型、关键 OOF、发布包、私有 Codex 冷备 | 是 |
...
Biohub 是历史 Kaggle 项目，且用户已经作出最终范围决定：`data/` 和所有 `data.partial*` 都是可重新下载的竞赛原始数据，**绝不上传**到 Drive、GitHub、临时 archive 或回收站迁移中。仓库保留其来源、版本、许可、下载方法和哈希即可；此前误启动的不完整远端 data 副本已移除。

`durable/` 则是用户自己的历史实验目录，和原始比赛数据不同。为避免 34 万级小文件与硬链接使直接上传耗时数天，第二波只保留一个完整 `tar.zst` 压缩档案（`bulk-biohub-durable-full-20260831.tar.zst`，传输/验收状态见 `BULK_MIGRATION_20260831.md`）。主 Biohub 仓库已有 main/worktree 快照、bundle、patch 和关键制品；任何清理前都必须先做 Git 快照和补丁，且不应把 `durable` 的历史候选结果自动当作当前研究输入。

### 2.3 Codex

`.codex` 的 7.2 GB 不等于 7.2 GB 的关键上下文。它包含 sessions、provider migration 备份、attachments、插件缓存、worktrees、配置和凭据。

保留：项目 `AGENTS.md`、必要配置说明、与当前研究直接有关的会话摘要/附件索引、关键任务背景，以及已完成的去凭据私有冷备。

经本机盘点，真正用于重新安装 Codex 后恢复关键工作方式的最小状态（去密配置之外的 goals/state/memories/queue 等）约为 **22–25 MB**；它与完整历史已按用户的紧急迁移授权以去凭据 `.tar.zst` 形式写入私有 Drive。完整会话历史约 3.23 GB、日志约 1.07 GB、provider migration 备份约 2.01 GB，均不是新智能体的默认阅读入口。

不进入 GitHub：`auth.json`、`secrets/`、Kaggle token、SSH 私钥、浏览器会话、插件缓存和不相关会话。

未来智能体以版本控制文档为准，完整 `.codex` 只做人工回溯的私有冷备。Codex 会读取仓库的 `AGENTS.md`，并允许更深目录覆盖通用规则，因此 `AGENTS.md` 应指定阅读顺序而非塞入全部历史。[OpenAI 官方说明](https://learn.chatgpt.com/docs/agent-configuration/agents-md)
...
├── README.md
├── AGENTS.md                             ← 新智能体启动规则
├── MIGRATION_RECEIPT_20260831.md          ← 已完成迁移的收据与恢复入口
├── SECURITY.md
├── docs/
│   ├── 00_人类阅读计划.md
│   ├── 01_研究总览.md
...
10_全量研究备份_20260831/             第二波；执行中的历史回溯层
  01_Project_研究工作区/               Project 广泛快照（上传中）
  02_Biohub_原始数据与历史/
    durable_archive/                   仅 Biohub durable 的完整压缩档案
  03_Omics_GPU_历史资料/               GO-AI、DiscoBAX、RNA transfer
  04_VirtualCODEX_完整研究档案/        仅用户个人内容；绝不含 mousebrain
  05_个人回收站研究恢复_20260831/      定向候选；尚不属于第二波统计
```
...
`10_全量研究备份_20260831/` 与前面的精选目录有不同的可信度：它是正在迁移的“完整回溯层”，每个顶层目录在 `rclone check` 完成前都不是已验证恢复包。其准确状态、恢复方式、硬性排除项和回收站候选只以 `BULK_MIGRATION_20260831.md` 为准；不要看到 Drive 中已有部分文件就把它写进核心 V7 manifest。
...
| `SENSITIVE_PRIVATE` | 会话、私密附件、必要状态 | 用户私有 Drive 冷备，不进 GitHub |
...
| **已制备研究包合计** | **1,641,096,976 B = 1.528 GiB** | 14 个包，完整清单与 SHA-256 见 `manifests/` |
| 去凭据 Codex 三包预快照 | 2,732,699,354 B = 2.545 GiB | 已上传并 checksum 校验；SQLite 已用 `.backup` 获取一致副本 |
| **全部已验证迁移包** | **4,373,796,330 B = 4.073 GiB** | 14 个研究包 + 3 个 Codex 包 |

结论：

- **可恢复研究并让新智能体接续的研究制品：1.528 GiB（加 GitHub 知识库后仍远低于 2 GiB）。**
- **BCR 已核实为结束且无优势的比较，因此只保留 2.84 MB 压缩结论证据；不搬 1.27 GB 内部 OOF。**
- **加上已上传的 Codex 预快照，实际已验证总量为 4.073 GiB。**如服务器仍可用，静默 Codex 后的小型最终 delta 是可选增强。

95 GB Biohub fallback、20+26 GB GO-AI runs、虚拟环境和服务器缓存均不进入**核心**迁移集。之后启动的第二波是独立、较宽的个人历史回溯计划（约 307.5 GiB），不能倒过来改变上述精选包的最小恢复结论：Biohub 原始 `data/` 与 `data.partial*` 仍完全排除，只有 `durable/` 的完整压缩档案可以进入第二波；他人的 `mousebrain/` 也完全排除。详见 `BULK_MIGRATION_20260831.md`。
...
7. 冻结报告：维护 `MIGRATION_RECEIPT_20260831.md`、未迁移清单和删除候选；未经人工批准不删除旧服务器内容。

## 9. 当前状态与尚余门槛

Google Drive 已在用户授权后验证可写，`00_迁移控制与清单/` 已创建；所有 17 个保留包都已上传、完成远端一致性检查并在 manifest 中标记为 `verified`。精确收据见 `MIGRATION_RECEIPT_20260831.md`。Codex 包按紧急授权直存用户私有 Drive，但认证材料仍未迁移；新机器应重新登录。
