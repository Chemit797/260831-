...
90_CODEX_PRIVATE_COLD_BACKUP/       去凭据、私有、加密后才可放完整历史
99_MANIFEST_SNAPSHOTS/              每一轮上传后的清单与校验和
```

每个二进制包采用 `*.tar.zst`（若完成用户掌握密钥的客户端加密，则为 `*.tar.zst.age` 或等价格式）。包内尽量保留原 `Project/...` 相对路径，因而可解压到新的项目根目录，而不是依赖服务器绝对路径。
...
* Codex 全史包含潜在敏感提示词和路径；只有在用户掌握的客户端加密密钥就绪后，才应上传完整原始历史。
