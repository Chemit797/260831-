...
class BaxAcquisition(BaseBatchAcquisitionFunction):
    def __init__(
        ...
        subset_select_num_samples: int = 20,
        device: str = "auto",
    ):
        ...
        self.subset_select_num_samples = subset_select_num_samples
        self.device = device
    ...
    def __call__(
        ...
        for j in range(self.num_samples_EIG):
            # load_folder returns a new model; assigning the return value is
            # required so every conditional fit starts at the same posterior.
            ...
            if self.objective_function == "topk":
                ...
                out = subset_select(
                    ...
                    self.subset_size,
                    budget=self.subset_select_num_samples,
                )
...
def noise_subset_select(
    noise_type, x, lengthscale=1.0, outputscale=1.0, device="auto"
):
    if noise_type == "additive":
        return lambda f: gaussian_noise_sampler(
            x, f, lengthscale, outputscale, device=device
        )
    elif noise_type == "multiplicative":
        return lambda f: bernoulli_noise_sampler(
            x, f, lengthscale, outputscale, device=device
        )
...
def gaussian_noise_sampler(
    ...
    with torch.no_grad():
        kernel = build_noise_kernel(lengthscale, outputscale, torch_device)
        cov = kernel(torch.as_tensor(x, dtype=torch.float32, device=torch_device))
    mean = torch.zeros(fx.shape, dtype=torch.float32, device=torch_device)
    eta = MultivariateNormal(mean, cov).sample().detach().cpu().numpy()
    return np.maximum(0, fx + eta)


def bernoulli_noise_sampler(
    x,
    fx,
    lengthscale=1.0,
    outputscale=1.0,
...
def build_noise_kernel(lengthscale, outputscale, device="cpu"):
    """Build the noise kernel and set its *actual* constrained parameters.

    GPyTorch's kernel constructors do not use ``lengthscale`` and
    ``outputscale`` as initial constrained values in the versions supported by
    this reproduction.  Assigning after construction is therefore deliberate.
    """
    kernel = ScaleKernel(RBFKernel()).to(device)
    kernel.base_kernel.lengthscale = float(lengthscale)
    kernel.outputscale = float(outputscale)
    return kernel
