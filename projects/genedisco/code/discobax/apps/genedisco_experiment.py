...
class GeneDiscoLoop(aba.AbstractBaseApplication):
    def __init__(
        ...
        bax_subset_select_num_samples: int = 20,
        num_topk_clusters: int = 20,
        device: str = "auto",
        scratch_directory: AnyStr = "",
    ):
        ...
        if scratch_directory:
            self.temp_folder_name = os.path.join(scratch_directory, self.run_name)
        else:
        ...
        self.bax_subset_select_num_samples = bax_subset_select_num_samples
        self.device = device
    ...
    def get_acquisition_function(self) -> BaseBatchAcquisitionFunction:
        if self.acquisition_function_name == "random":
            ...
            return bax_sampling.BaxAcquisition(
                ...
                subset_select_num_samples=self.bax_subset_select_num_samples,
                device=self.device,
            )
                ...
                record = ",".join(
                    [
                        ...
                        for x in [
                            ...
                            self.bax_subset_select_num_samples,
                            self.device,
                        ]
                    ]
                )
